...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word] + [self.char2id.get(char, self.char_unk) for char in word] + [self.end_of_word]
                 for word in s] for s in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_char_indeces = pad_sents_char(char_indeces, self.word2id['<pad>'])
        tmp = torch.Tensor(padded_char_indeces) # shape - batch_size, max_sentence_length, max_word_length
        return tmp.permute(1, 0, 2).long().to(device=device)
        ### END YOUR CODE
